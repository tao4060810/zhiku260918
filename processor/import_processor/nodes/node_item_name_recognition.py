...
class NodeItemNameRecognition(BaseNode):
    ...
    def _step_2_build_context(self, chunks: List[Dict]) -> str:
        ...
        parts: List[str] = []
        total_chars = 0
        for idx, chunk in enumerate(chunks[:import_config.item_name_chunk_k], start = 1):
            ...
            # 4. 判断是否需要继续切分
            if total_chars > import_config.item_name_chunk_size:
                logger.warning(f"累计字符数{total_chars}已超过限制{import_config.item_name_chunk_size}，停止切分")
                break
        ...
        # 6. 对返回结果进行二次截断
        final_context = context[:import_config.item_name_chunk_size]
...
if __name__ == "__main__":
    ...
    with open(r"D:\output\hak180产品安全手册\chunks_new.json", "w", encoding="utf-8") as f:
