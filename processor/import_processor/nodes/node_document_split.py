...
class NodeDocumentSplit(BaseNode):
    ...
    def _split_long_section(self, section: Dict[str, str]) -> List[Dict[str, str]]:
        ...
        # 1. 长度未超限，无需切分，直接返回
        if len(content) <= import_config.max_content_length:
            return [section]
        ...
        # 计算正文的长度
        available_len = import_config.max_content_length - len(prefix)

        if available_len <= 0:
            logger.warning("章节标题过长，无法切分")
            return [section]
    ...
    def _merge_short_sections(self, sections: List[Dict[str,str]]) -> List[Dict[str,str]]:
        ...
        for section in sections:
            ...
            #段落是否过短
            is_current_short = len(current_chunk["content"]) < import_config.min_content_length
