/* 两个页面共用导入状态：上传、排队、解析、入库期间锁定界面，只允许查看进度。 */
const importState = {checking:true, uploading:false, uploadText:'', tasks:[], batch:new Set(), error:'', timer:null, version:0, stopped:false};
const importActive = task => ['pending','processing'].includes(task.status);
function isImportLocked() {
  return importState.checking || importState.uploading || importState.tasks.some(importActive);
}
function renderImportStatus() {
  ...
  $('#import-status-description').textContent = importState.uploading ? importState.uploadText : '导入期间暂时无法进行其他操作，全部完成或失败后将自动恢复。';
...
async function refreshImportTasks() {
  ...
  try {
    ...
    if (wasActive && !data.items.some(importActive) && !importState.uploading) {
      ...
      toast(failed ? `导入已结束，${failed} 份文档失败，请在文档管理中查看原因。` : '导入已结束，可以继续操作。');
      importState.batch.clear();
    }
  } catch (error) {
    if (version !== importState.version || importState.stopped) return;
